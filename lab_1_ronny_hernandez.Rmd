...
<!-- Import random ratio sample data -->
...
<!-- Import random reflectance  -->

```{r impor_random_reflectance}
files <- dir_ls(path = "data",  glob = "data/random_reflectance*")

random_reflectance <- map_dfr(.x = files, 
                              .id = "file_id",
                              .f = function(archivo) {
                                read_csv(archivo,
                                         skip = 4,
...
# Rock 2
files <- dir_ls(path = "data/", glob = "data/meta*")

rock_2 <- map_dfr(.x = files, 
                  .id = "file_id",
                  .f = function(archivo) {
                    read_csv(archivo,
                             skip = 4,
...
<!-- Import spectra correction data -->

```{r import_spectra_correction}
...
calculate_ndvi <- function(data) {
  ...
  red <- data %>% 
    ...
    filter(wavelength_nm >= 625 & wavelength_nm <= 700) %>% 
    select(reflectance) %>%
    pull() %>% 
    mean()
  ...
  nir <- data %>% 
    select(wavelength_nm, reflectance) %>% 
    filter(wavelength_nm >= 750 & wavelength_nm <= 900) %>% 
    select(reflectance) %>%
    pull() %>% 
    mean()
...
In the case of random samples, we present the results for ratios, reflectance 
and the first and second derivative. Each section result is showed below,
naming each for the values from each data file as follows:

 - Values from file `001-00006.spu` are categorized as `random_06`
 - Values from file `001-00018.spu` are categorized as `random_18`
 - Values from file `001-00023.spu` are categorized as `random_23`
...
### Random samples reflectance

```{r random_sample_reflectance, fig.cap = "Reflectances for the random samples"}
random_reflectance %>% 
  ...
  ggplot(aes(x = wavelength, 
             ...
             color = file_id)) +
  geom_line() +
  scale_y_continuous(breaks = seq(0, 1.13, by = 0.1)) +
  scale_x_continuous(breaks = seq(400, 900, by = 50)) +
  scale_color_viridis_d(labels = c("random_06", "random_18", "random_23")) +
...
rock_1 %>% 
  ...
  ggplot(aes(x = wavelength, y = reflectance, color = file_id)) +
  geom_line() +
  scale_color_viridis_d(labels = c("Rock sample 1",
                                   "Rock sample 2",
                                   "Rock sample 3")) +
  ...
  labs(x = "Wavelength (nm)",
       y = "Reflectance",
       color = "Rock sample position") +
  theme_light(base_size = 11)
```

```{r rock_2_reflectance, fig.cap = "Reflectance of rock 2."}
rock_2 %>% 
  filter(wavelength > 450, wavelength < 900) %>% 
  ggplot(aes(x = wavelength, y = reflectance, color = file_id)) +
  geom_line() +
  scale_color_viridis_d(labels = c("Rock sample 1",
                                   "Rock sample 2",
                                   "Rock sample 3")) +
  scale_y_continuous(breaks = seq(0, 2.6, by = 0.01)) +
  scale_x_continuous(breaks = seq(400, 1000, by = 50)) +
  labs(x = "Wavelength (nm)",
       y = "Reflectance",
       color = "Rock sample position") +
...
```{r spectra_correction_plot, fig.cap = "Values for reflectance, reference and dark current from the spectra correcction example"}
...
```{r}
ndvi <- calculate_ndvi(spectra_correction)
```

The estimated ndvi is `r ndvi`
